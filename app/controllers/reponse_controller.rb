class ReponseController < ApplicationController
  ...
  def index
    ...
    @couleurs = Reponse.distinct.pluck(:couleur)
    @prices = Pricesreponse.all
    @mynombre = params[:nombre]
    @mynombreCR = params[:nombre]
  ...
  #   params.require(:fairepart).permit(:nom, :slug, :couleur, :format, :verso, :image)
  # end

  def filter_reponses
    if @mycouleur.present? && @myverso.present?
  ...
  def filter(val1, val2)
    ...
    @reponses = @reponses.where("#{val2} LIKE ? OR #{val2} LIKE ?", true_variable, false_variable)
